...
impl Game {
    pub fn launch(&self) {
        if self.store == "Steam" {
            ...
            let run_string: String = format!(
                "{}{}{}",
                EPIC_GAMES_LAUNCH_PREFIX, self.appid, EPIC_GAMES_LAUNCH_SUFFIX
            );
            open::that(run_string).unwrap();
        }
...
fn get_egsgames() -> Vec<Game> {
    ...
    for f in folders {
        ...
        let files = fs::read_dir(next_path)
            .unwrap()
            .next()
            .unwrap()
            .unwrap()
            ...
            .into_string()
            .unwrap();
        let id_path: ReadDir = fs::read_dir(files).unwrap();

        for file in id_path {
            let entry: DirEntry = file.unwrap();
            let isdir: bool = entry.file_type().unwrap().is_dir();
            if !isdir {
                if entry.path().extension().as_ref().unwrap() == &"mancpn" {
                    let file_to_read: String =
                        entry.path().as_os_str().to_owned().into_string().unwrap();
